package me.shroomz.voterewards.Scoreboard;
...
public class ScoreboardHandler {
    ...
    public void reload() {
        objective.setDisplayName(ChatColor.translateAlternateColorCodes('&',
                Voterewards.getPlugin().getConfig().getString("leaderboard.scoreboard.title", "Top voters")));
    }

    public void doPopulate() {
        if (!Voterewards.getPlugin().getConfig().getString("leaderboard.display").equals("scoreboard")) {
            return;
        }
        List<PlayerVotes> leaderboardAsUuids = Voterewards.getPlugin().getVoteStorage().getTopVoters(
                Math.min(16, Voterewards.getPlugin().getConfig().getInt("leaderboard.scoreboard.max", 10)), 0);

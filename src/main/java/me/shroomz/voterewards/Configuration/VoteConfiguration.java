...
import me.shroomz.voterewards.Commands.CommonCommand;
import me.shroomz.voterewards.Configuration.Message.OfflineVoteMessages;
...
import me.shroomz.voterewards.Configuration.Message.VoteMessages;
import me.shroomz.voterewards.Storage.JsonVoteStorage;
...
import me.shroomz.voterewards.Votes.Rewards.Matchers.RewardMatchers;
import me.shroomz.voterewards.Votes.Rewards.Matchers.StaticRewardMatcher;
...
public class VoteConfiguration {
    ...
    public VoteConfiguration(ConfigurationSection section) {
        ...
        try {
            ...
            Voterewards.getPlugin().getLogger().log(Level.SEVERE, "Unable to load your reward configuration.", e);
            Voterewards.getPlugin().getLogger().severe("Voterewards will be disabled until your configuration is fixed.");
            configurationError = true;
        }

        if (rewards.isEmpty() && !configurationError) {
            Voterewards.getPlugin().getLogger().severe("Your configuration does not specify any rewards.");
            configurationError = true;
        }
        ...
        if (defaultRewardCount == 0 && !configurationError) {
            Voterewards.getPlugin().getLogger().severe("No default reward was defined. To set a default reward, set default: true in one of your reward if blocks.");
            configurationError = true;
        } else if (defaultRewardCount > 1 && !configurationError) {
            Voterewards.getPlugin().getLogger().severe("Multiple default rewards are defined. Hint: You may want to check the spelling in your 'if' blocks. (Further information has been logged.)");
            configurationError = true;
        }
        ...
        if (rewardsNotProperlyConfigured) {
            Voterewards.getPlugin().getLogger().severe("Some of your rewards are not correctly set up. This could cause issues, so Voterewards will not work until you have fixed your configuration. (Further information has been logged.)");
            configurationError = true;
        }
    ...
    public VoteStorage initializeVoteStorage() throws IOException {
        ...
        if (!SUPPORTED_STORAGE.contains(storage)) {
            Voterewards.getPlugin().getLogger().info("Storage method '" + storage + "' is not valid, using JSON storage.");
            storage = "json";
        }
        ...
        switch (storage) {
            case "json":
                ...
                if (file == null) {
                    file = "votes.json";
                    Voterewards.getPlugin().getLogger().info("No file found in configuration, using 'votes.json'.");
                }
                return new JsonVoteStorage(new File(Voterewards.getPlugin().getDataFolder(), file));
            case "mysql":

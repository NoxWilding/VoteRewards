    ...
    public void run() {
        for (TopPlayerSign sign : toUpdate) {
            ...
            if (location.getWorld() == null) {
                Voterewards.getPlugin().getLogger().severe("World for sign " + sign.getSign() + " is missing!");
                continue;
            }
            ...
            if (block.getState() instanceof Sign) {
                ...
                if (sign.getPosition() > top.size()) {
                    ...
                    int lines = Voterewards.getPlugin().getConfiguration().getTopPlayerSignsConfiguration().getSignText().size();
                    for (int i = 0; i < Math.min(4, lines); i++) {
                        PlainStringMessage m = Voterewards.getPlugin().getConfiguration().getTopPlayerSignsConfiguration().getSignText().get(i);

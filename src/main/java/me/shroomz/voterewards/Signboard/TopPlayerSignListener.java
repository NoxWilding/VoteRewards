...
public class TopPlayerSignListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        for (TopPlayerSign sign : Voterewards.getPlugin().getTopPlayerSignStorage().getSignList()) {
    ...
    private boolean doSignBreak(Player player, TopPlayerSign sign) {
        ...
        Voterewards.getPlugin().getTopPlayerSignStorage().removeSign(sign);
    ...
    public void onSignChange(SignChangeEvent event) {
        try {
            if (event.getLine(0).startsWith("[topvoter]") && event.getPlayer().hasPermission("superbvote.managesigns")) {
                ...
                Voterewards.getPlugin().getTopPlayerSignStorage().addSign(sign);
                updateSigns();
    ...
    private void updateSigns() {
        Bukkit.getScheduler().runTaskAsynchronously(Voterewards.getPlugin(), new TopPlayerSignFetcher(
                Voterewards.getPlugin().getTopPlayerSignStorage().getSignList()));
    }
}